...
Original file is located at
    https://colab.research.google.com/drive/1K-rnqU_GlunItnltE2WIl9MaC4v6x9qX
"""
...
mclip = pt_multilingual_clip.MultilingualCLIP.from_pretrained(TEXT_MODEL_ID, clip_model_id=CLIP_VISION_ID).to(DEVICE).eval()
...
def score_image_captions(image, captions):
    ...
    txt_tok = mclip_tok(captions, padding=True, truncation=True, return_tensors="pt")
    txt_tok = {k: v.to(DEVICE) for k, v in txt_tok.items()}

    embs = mclip.transformer(**txt_tok)[0]
    att_mask = txt_tok["attention_mask"]
    embs = (embs * att_mask.unsqueeze(2)).sum(dim=1) / att_mask.sum(dim=1, keepdim=True)
    txt_feat = mclip.LinearTransformation(embs)
